...
class a2svm(object):
    ...
    def load(self, config_id):
        if self.config.has_section(config_id):
            ...
            print(
                "Please provide informations about your Apache configuration: ".center(
                    50, "+"
                )
            )
            ...
            if save_config:
                ...
                if input_macro_file_filter:
                    self.config.set(
                        config_id, "macro_file_filter", int(input_macro_file_filter)
                    )
                else:
                    self.config.set(
                        config_id, "macro_file_filter", self.macro_file_filter
                    )
                if input_vhost_config_path:
                    self.config.set(
                        config_id, "vhost_config_path", input_vhost_config_path
                    )
                else:
                    self.config.set(
                        config_id, "vhost_config_path", self.vhost_config_path
                    )
                if input_vhost_enabled_path:
                    self.config.set(
                        config_id, "vhost_enabled_path", input_vhost_enabled_path
                    )
                else:
                    self.config.set(
                        config_id, "vhost_enabled_path", self.vhost_enabled_path
                    )
                if input_vhost_enabling_command:
                    self.config.set(
                        config_id,
                        "vhost_enabling_command",
                        input_vhost_enabling_command,
                    )
                else:
                    self.config.set(
                        config_id, "vhost_enabling_command", self.vhost_enabling_command
                    )
                if input_vhost_disabling_command:
                    self.config.set(
                        config_id,
                        "vhost_disabling_command",
                        input_vhost_disabling_command,
                    )
                else:
                    self.config.set(
                        config_id,
                        "vhost_disabling_command",
                        self.vhost_disabling_command,
                    )
                if input_apache_reload_command:
                    self.config.set(
                        config_id, "apache_reload_command", input_apache_reload_command
                    )
                else:
                    self.config.set(
                        config_id, "apache_reload_command", self.apache_reload_command
                    )
    ...
    def list(self):
        ...
        print("-" * 119)
        print(
            "| {0:20}| {1:20}| {2:8}| {3:30}| {4:30}|".format(
                "Name", "Macro", "Enabled", "ServerName", "Directory"
            )
        )
        print("-" * 119)
    ...
    def make(self, vhost, opt_args):
        ...
        if os.path.isfile(os.path.join(self.macro_path, vhost.macro + ".conf")):
            pass
        else:
            print(
                "Error, macro file not found, are you sure "
                + os.path.join(self.macro_path, vhost.macro + ".conf")
                + " exist?"
            )
            sys.exit(1)
        ...
        opt_args_content = ""
        print(
            "The vhost will be created using the macro named '"
            + vhost.macro
        ...
        if (len(opt_args) + 3) != (macro_arg_number):
            print(
                "Error, this macro require "
                + str(macro_arg_number)
                + " arguments but "
                + str(len(opt_args) + 3)
                + " founds"
            )
...
def launcher():
    ...
    parser_mk = subparsers.add_parser(
        "mk", description="Create a vhost", help="Create a vhost"
    )
    parser_mk.add_argument(
        "mk_vhost_name", metavar="<vhost_name>", type=str, help="Name of the vhost"
    )
    parser_mk.add_argument(
        "mk_vhost_macro",
        metavar="<vhost_macro>",
        type=str,
        help="Macro used by the vhost",
    )
    parser_mk.add_argument(
        "mk_vhost_servername",
        metavar="<vhost_servername>",
        type=str,
        help="ServerName of the vhost",
    )
    parser_mk.add_argument(
        "mk_vhost_directory",
        metavar="<vhost_directory>",
        type=str,
        help="Directory of the vhost",
    )
    parser_mk.add_argument(
        "mk_opt_args",
        metavar="[optionnal_macro_arg]",
        nargs="*",
        type=str,
        help="Optionnal macro argument",
    )

    parser_ls = subparsers.add_parser(
        "ls",
        description="Show vhost on Apache server",
        help="Show vhost on Apache server",
    )
    ...
    parser_tls.add_argument(
        ...
        action="store_true",
        dest="tls_all",
        help="Request certificate for all enabled vhosts",
    )
    parser_tls.add_argument(
        "tls_vhost_name",
        metavar="<vhost_name>",
